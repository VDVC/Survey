...
data = pd.read_csv('rohdaten/daten2013.dat',sep=";")

hist_alter=(2013-(data[data.Geburtsjahr>1950])['Geburtsjahr']).value_counts()
...
hist_alter.to_csv("./daten/2013/alter.tsv",sep='\t',
...
pd.DataFrame(data=hist_game,\
             columns=["Anzahl"]).to_csv("./daten/2013/titel.tsv",\
                                           sep='\t',
...
pd.DataFrame(data=hist_freigabe,\
             columns=["Anzahl"]).to_csv("./daten/2013/freigabe.tsv",\
                                           sep='\t',
                                           quoting=csv.QUOTE_NONNUMERIC,
                                           index_label=["Freigabe"])

freigabe_alter=pd.crosstab(2013-(data[data.Geburtsjahr>1950])['Geburtsjahr'],\
               data.Freigabe1)\
              +pd.crosstab(2013-(data[data.Geburtsjahr>1950])['Geburtsjahr'],\
               data.Freigabe2)\
              +pd.crosstab(2013-(data[data.Geburtsjahr>1950])['Geburtsjahr'],\
               data.Freigabe3)\
              +pd.crosstab(2013-(data[data.Geburtsjahr>1950])['Geburtsjahr'],\
...
freigabe_alter["Gesamt"]=freigabe_alter["."]\
                        ...
                        +freigabe_alter["USK6"]
pd.DataFrame(data=freigabe_alter).to_csv("./daten/2013/freigabe-alter.tsv",\
                                           sep='\t',
                                           quoting=csv.QUOTE_NONNUMERIC,
                                           index_label=["Alter"])
